...
function getFinals(data) {
   const finalsData = data.filter(x => x.Stage === 'Final' ? x : null )
   return finalsData
}
getFinals(fifaData)
...
function getYears(data, cb) {
    const years = cb(data).map(x => x.Year)
    return years
}
...
function getWinners(data, cb) {
    const winners = cb(data).map(x => x['Home Team Goals'] >= x['Away Team Goals'] ? x['Home Team Name'] : x['Away Team Name'])
    return winners
}
...
// getWinners(fifaData, getYears, getWinners)
